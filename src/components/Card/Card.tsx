...
import { Quote, SavedQuote } from '../../context/dataTypes';
...
function Card({ data, topCard, className }: CardProps) {
  ...
  const { savedQuotes, setCurrentQuote, setBookmarkModalVisible } = useStateContext();
  const saved = savedQuotes.find(v => v.quoteId === data._id)

  const handleBookmarkClick = () => {
    setCurrentQuote({ savedQuote: saved, quote: data });
  ...
  return (
    ...
    relative group cursor-pointer bg-white rounded-lg shadow-lg xsm:h-[300px] xs:h-[260px] h-[240px] text-center p-5`}>
      ...
      <BookmarkButton large={topCard} onClick={handleBookmarkClick} active={saved !== undefined}
