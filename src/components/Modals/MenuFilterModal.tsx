...
function MenuFilterModal({ visible }: MenuModalProps) {
  ...
  const onFilterClick = (index: number, _all: boolean) => {
    let all = _all;
    if (!filters[index].active && (filters.length - 1 === activeFilters.length))
      all = true;
    ...
    <MenuModal visible={visible} className='max-[450px]:-left-4 max-[450px]:translate-x-0'>
      <>
        <li className='border-b border-black/30'
          onClick={() => onFilterClick(0, true)}>
          <div className='xs:py-1 py-1.5 flex opacity-60 hover:opacity-100 capitalize'>
            <Checkbox className='mr-4' active={activeFilters.length === 0} />All
          </div>
        </li>
        {
          filters.map((f, i, row) => {
